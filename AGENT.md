...
## C Projects (c-samples, adguard-dashboard)
Two projects are C/C++, and they must not build against a sibling checkout:
...
- **Board deploy**: `scripts/start-dashboard.sh` kills any previous instance, starts detached, logs to `/tmp/agh-dash.log` and fails loudly if the @sgc daemon is not running. The daemon must be up first — an @sgc client is sgc-or-die.
- **Verify by reading pixels, not by looking at the screen.** `--self-check` lets a couple of refreshes land, then reads the display's active buffer back (`lv_display_get_buf_active`) and prints per-widget geometry and pixel counts, with a table's text pixels as a control. That is how "the charts don't show" turned out to be a data-slice bug rather than a rendering one.
