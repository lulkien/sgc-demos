...
| `ir-remote-test` | DRM + Slint (Cargo) | `backend-linuxsgc`, `femtovg` (GPU only), `input` (in default) | An IR remote test screen: shows the last key and the last 10 events the seat delivers (`seat` rows) beside an optional raw-evdev read-back (`raw` rows, `--raw [device]`) for the keycodes the xkb keysym mapping drops. Ships the remote's keymap + the systemd unit that loads it + `docs/ir-keymap.md` (install, extend, port). Test tool, not an application — see the project README for why the remote's device is not in the daemon's input inventory without that map |
...
- `ir-remote-test` is run **instead of** a dashboard, not beside it: one DRM lease, one seat. `--raw [device]` adds the diagnostic evdev read-back
...
- ❌ Do NOT use `.unwrap()` on the font file read — its existence is a build-time invariant; use `expect()` with a doc comment if the path might change
...
- ❌ Do NOT request a resource the app cannot consume — the LVGL dashboard sets `LV_SGC_INPUT 0` and the Slint one keeps `input` out of its default features for the same reason; holding what you cannot use takes it from other clients. When a UI does need input, enable it deliberately (the Slint backend then draws the cursor and quits on Ctrl+Alt+Backspace). `ir-remote-test` is the deliberate exception: it exists to consume input, so it carries `input` in its default features
- ❌ Do NOT compile the software renderer in as a revoke fallback — the fork's linuxsgc backend survives a revoke on the GL path, so a CPU renderer only hides a broken GL userspace (`ir-remote-test` is GPU-only for this reason; the dashboards keep `software` for their offscreen `--self-check`)
