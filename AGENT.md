...
| `input-harness` | test tooling (no client) | `uinput-inject`, `sgc-steal` | Virtual keyboard/mouse/touchscreen via `/dev/uinput` plus a resource steal probe, driving the revoke/re-grant cycle; `scripts/input-resume-test.sh` asserts the running daemon adopts the virtual device (no restart), that input delivery survives a re-grant (negative control included), and that the daemon withdraws the device once its injector dies |
