...
| `adguard-dashboard-slint` | DRM + Slint (Cargo) | `backend-linuxsgc`, software renderer, no input | The same dashboard as a Slint UI: same layout, same `/etc/agh-dash/config.toml`, worker thread for the API, offscreen `--self-check` that counts pixels |
...
- **Slint specifics.** The Rust dashboard renders on the **GPU** by default: the crate enables the fork's `backend-linuxsgc` feature with the `femtovg` renderer (OpenGL over gbm/EGL), and the backend itself is `try_femtovg_then_software` — GL first, CPU renderer as the fallback when GL init fails. A lease revoke is **fatal for the GL flavor** (the EGL/GL context is created on the lease fd), so a steal kills the process: the launcher restarts it on demand, and a unit with `Restart=always` is the way to make it come back on its own. `--no-default-features --features software` builds the CPU flavor, which parks and resumes across a revoke instead. Both renderers stay compiled in so the offscreen `--self-check` can use the software path while the window uses the GPU. It also takes no `input` feature — read-only app, and a client must not hold a device it cannot consume. The board image has neither fontconfig nor freetype, so the app registers `DejaVuSans.ttf` into fontique at startup (what `slint-lease-client` does too); Slint is event-driven on one thread, so the fetch loop lives on a worker thread and the UI thread only drains a channel.
...
- ❌ Do NOT request a resource the app cannot consume — the dashboard takes no input, so its `lv_conf.h` sets `LV_SGC_INPUT 0`; holding what you cannot use takes it from other clients
